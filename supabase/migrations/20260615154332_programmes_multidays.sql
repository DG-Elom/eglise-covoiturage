...
-- Check : chaque élément entre 0 et 6 (drop d'abord, pas de "add constraint if not exists" portable).
-- Pas de sous-requête possible dans un CHECK → on utilise l'opérateur "contained by" (<@).
-- Un array vide est contenu dans n'importe quel array → OK pour les programmes-événements.
alter table cultes
  drop constraint if exists cultes_jours_semaine_values;
alter table cultes
  add constraint cultes_jours_semaine_values
  check (jours_semaine <@ array[0,1,2,3,4,5,6]::smallint[]);
...
begin
  ...
  where  t.actif = true
    ...
    and  extract(dow from d)::int = any(c.jours_semaine::int[])
